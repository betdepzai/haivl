...
$errors = array();
$show_form = true;
$form_id = md5(time().$_SERVER['REMOTE_ADDR']);
...
if ($_POST['btn-pressed'] == 'delete')
{
	...
	else if ( ! csrfguard_validate_token(substr(md5('_edit_video_form'.$_POST['form_id'].$userdata['id'].$_SERVER['REMOTE_ADDR']), 3, 8), $_POST['_pmnonce_t_edit_video_form']))
	{
...
if ($_POST['btn-pressed'] == 'submit')
{
	...
	if ( ! csrfguard_validate_token(substr(md5('_edit_video_form'.$_POST['form_id'].$userdata['id'].$_SERVER['REMOTE_ADDR']), 3, 8), $_POST['_pmnonce_t_edit_video_form']))
	{
		$errors[] = $lang['upload_errmsg_badtoken'];
	}
	else
...
$smarty->assign('form_csrf', csrfguard_raw( substr(md5('_edit_video_form'.$form_id.$userdata['id'].$_SERVER['REMOTE_ADDR']), 3, 8) ));
