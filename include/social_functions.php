...
function get_followers_list($leader_id, $start = 0, $limit = 30)
{
	...
	$sql = "SELECT f.follower_id, u.username, u.name, u.gender, u.country, u.reg_date, u.last_signin, u.power, u.about, u.avatar, u.followers_count, u.following_count, u.channel_slug, u.channel_verified 
 ...
 * 
 * @todo add 'total videos uploaded' to the pot
 * @since 2.1
...
function suggest_who_to_follow($start_from = 0, $limit = 10)
{
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$row['followers_count_formatted'] = pm_number_format($row['followers_count']);
		$row['following_count_formatted'] = pm_number_format($row['following_count']);
		...
		$user_list[$id]['profile_url'] = get_profile_url($row);
		...
		{
			$user_list[$id]['channel_cover'] = array('filename' => $row['channel_cover'],
