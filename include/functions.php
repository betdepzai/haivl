...
function load_categories($args = array())
{
	...
	while ($row = mysql_fetch_assoc($result))
	{
		if ($db_table == 'pm_categories' && $with_image)
		{
			...
			else if ($row['published_videos'] > 0)
			{
...
function serve_preroll_ad($page = '', $video_data = false)
{
	...
	if ($page == 'detail' && $total_video_ads > 0)
	{
		...
		$video_ad_data = array();
		$sql = "SELECT id, hash, redirect_type 
				FROM pm_videoads 
				WHERE status='1' 
				ORDER BY RAND() 
				LIMIT 1";
...
/**
 * Checks if the SSL is enabled based on the _URL constant 
 * and defines _SSL_ENABLED constant if it doesn't already exist
 * 
 * @since 2.6
 * @return bool 
 */
function ssl_enabled()
{
	...
	if (defined('_SSL_ENABLED') && constant('_SSL_ENABLED') == true)
	{
		return _SSL_ENABLED;
	}
	
	$parsed_url = parse_url(_URL);
	$is_ssl = ($parsed_url['scheme'] == 'https' || $_SERVER['HTTPS'] == 'on') ? true : false;
	
	if ( ! defined('_SSL_ENABLED'))
	{
		define('_SSL_ENABLED', $is_ssl);
	}
