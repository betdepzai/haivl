...
function show_edit_cron_job_form($job = false)
{
	...
	<div>
		...
		<div class="control-group">
			<label>Exclude search keywords</label>
		...
		<div class="control-group">
			...
			<?php if ($config['total_videos'] > 30000 && $job['type'] == 'vscheck') : ?>
...
function show_cron_job_state_html($job)
{
	...
	switch ($job['state'])
	{
		...
		case CRON_STATE_BUSY:
			?> 
			<span class="label label-warning" rel="tooltip" title="Job in progress">working</span>
			<?php
		break;
		...
		case CRON_STATE_READY:
			...
			switch ($job['status'])
			{
				case CRON_STATUS_LIVE:
					...
					if ($job['type'] == 'vscheck' && $job['data']['time_started'] > 0)
					{
						?>
						<span class="label label-warning" rel="tooltip" title="Job in progress">working</span>
						<?php
