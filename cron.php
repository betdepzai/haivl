...
switch ($job['type'])
{
	case 'import':
		...
		try {
			...
			cron_exit_with_error('PHP Error: '. $e->getMessage() ."\n<br />File: ". $e->getFile() ."\n<br />Line: ". $e->getLine());
		}
		
		if ($response->code != 200)
		{
			if ($job['data']['search_request_attempts'] <= 3)
			{
			...
			{
				cron_exit_with_error('Failed to fetch search data. The server responded with a <code>'. $response->code .'</code> multiple times.');
			}
		}
		else
		{
		...
		if (count($search_data['items']) > 0)
		{
			...
			foreach ($search_data['items'] as $k => $item)
			{
 			...
 			try {
				...
				cron_exit_with_error('Error: '. $e->getMessage() ."\n<br />File: ". $e->getFile() ."\n<br />Line: ". $e->getLine());
			}
			
			if ($response->code != 200)
			{
				// try again next time
				cron_log('Failed to send the import request. The server responded with a <code>'. $response->code .'</code>.');
				cron_end();
			}
